...
## Mandatory OCR and OpenAI fallback
...
```bash
--ocr --require-ocr --openai-ocr --openai-ocr-live --require-openai-ocr
```

This prevents the UI from silently producing a non-OCR result or skipping the approved OpenAI fallback on OCR-heavy medical record batches.
